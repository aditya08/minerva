# Copyright 2020. Aditya Devarakonda
...
class Account(object):
    def __init__(self, name, assets=[]):
        ...
        Args:
            ...
            assets (list, optional): List of assets to add to account. Defaults to [].
        """
        self.name = name
        self.value = 0
        self.assets = []
    ...
    def __str__(self):
        ...
        for asset in self.assets:
            acct_str += asset.ticker + '- [quantity]: ' + asset.quantity \
                        + ' [price]: ' + asset.price + ' [basis]: ' + asset.basis \
                        + '\n'
        return acct_str
    ...
    def __iadd__(self, other):
        if isinstance(other, Asset):
            idx = [asset.ticker for asset in self.assets].index(other.ticker.lower())
    ...
    def buy_assets(self, assets=[], path='', replace=False):
        ...
        through `assets` arg or as a json file through the `path` arg. Mixing of the two
        input args is allowed. Also takes a Finnhub API key to update Asset prices and
        descriptions. If the `replace` flag is True, then existing Assets in the Account
        object are overwritten. If False, then existing Assets are modified by adding
        quantities and by adjusting the cost basis.


        Args:
            assets (list, optional): A list of Assets (e.g. Stocks, Funds). Defaults to [].
            path (str, optional): path to a json file containing Assets. Defaults to ''.
            replace (bool, optional): If True, existing Asset will be overwritten. Otherwise, existing Asset
            will be modified to reflect the additional quantity and computes a new cost basis. Defaults to False.
    ...
    def _buy_assets_from_list(self, assets, replace=False):
        ...
        for asset in assets:
            if not isinstance(asset, Asset):
                raise TypeError('unexpected type {} found for asset.'.format(type(asset)))
    ...
    def _buy_asset(self, idx, asset, replace=False):
        ...
        if isinstance(found_asset, Fund) and replace:
            ...
            # TODO: Finnhub does not provide this info for Funds (use yfinance? quandl?)
    ...
    def _sell_asset(self, idx, asset):
        ...
        if found_asset.quantity < asset.quantity:
            raise ValueError('quantity ({}) to sell exceedes quantity ({}) available in account.'
                            .format(asset.quantity, found_asset.quantity))
        adj_quantity = found_asset.quantity - asset.quantity
        adj_basis = (found_asset.quantity*found_asset.basis -
                        asset.quantity*asset.basis)/adj_quantity
        if adj_basis < 0:
            raise ValueError('basis of selling asset exceeds basis in account.')
        found_asset.update_quantity(adj_quantity)
        found_asset.update_basis(adj_basis)
    ...
    def update(self):
        ...
        for asset in self.assets:
            ...
            price = float(scraper.quote(asset.ticker)['Previous close'].strip('$'))
    ...
    def _create_new_asset_from_dict(self, asset):
        if asset['type'].lower() == 'stock':
                ...
                sector=asset['sector'], description=asset['description'])
            ...
            asset['expense_ratio'], asset['sector'], asset['description'])
        else:
